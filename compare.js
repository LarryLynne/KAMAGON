...
function buildCompareTableHTML(vehicleRows, planFleet, rduFleet, datesList, planOps, autoFactOps) {
    ...
    datesList.forEach(dStr => {
        ...
        html += `<th colspan="26" style="text-align: center; font-weight: bold; background-color: #e9ecef; border-left: 2px solid #6c757d; border-right: 2px solid #6c757d; padding: 4px 0;">
            ${dStr}<br><span style="font-size: 11px; font-weight: normal; color: #6c757d;">${dayName}</span>
        </th>`;
    });
    ...
    datesList.forEach(dStr => {
        ...
        html += `<th colspan="26" style="text-align: center; font-weight: bold; background-color: #f8fafc; border-left: 2px solid #6c757d; border-right: 2px solid #6c757d; padding: 4px 0;">
            ${dStr}<br><span style="font-size: 11px; font-weight: normal; color: #6c757d;">${dayName}</span>
        </th>`;
    });
    ...
    // Рядок План
    html += `<tr><td class="sticky-col" style="font-weight:bold; background-color:#fff;">Operational (План)</td>`;
    let grandPlanOps = 0;
    datesList.forEach(dStr => {
        let daySum = 0;
        for(let h=0; h<24; h++) {
...
window.exportCompareToExcel = async function(workbook) {
    ...
    if (!startVal || !endVal) {
        alert("Оберіть період дат для експорту звірки!");
        return false;
    }
    ...
    try {
        ...
        bulkData = await fetch(url).then(r => r.json());
    } catch (err) {
        console.error("Помилка скачування bulk-даних:", err);
        alert("Не вдалося завантажити загальний пакет даних з Google Таблиць!");
    ...
    if (exportMode === 'current') {
        ...
        if (bulkData.rduRows) bulkData.rduRows.forEach(r => yardsSet.add(r[0]));
        yardsToExport = Array.from(yardsSet).sort();
    }

    if (yardsToExport.length === 0) {
        alert("Не знайдено жодних розрахованих даних за вказаний період!");
        btn.innerText = originalText; btn.disabled = false;
        return false;
    }
    ...
    for (const yard of yardsToExport) {
        ...
        findMaxFleetBounds(yardPlanRows);
        findMaxFleetBounds(yardRduRows);
        ...
        sheet.getCell(3, totalPlanColIdx).value = "Разом План";
            ...
            const tempChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        ...
                        { type: 'line', label: 'Тр. РДУ', data: autoFactCap[dStr], borderColor: '#dc2626', borderWidth: 2, pointRadius: 1, tension: 0.1 }
                    ]
                },
                options: {
    ...
    if (totalSheetsCreated === 0) {
        alert("Немає розрахованих даних звірки для експорту за обраний період!");
        return false;
    }
    return true;
};